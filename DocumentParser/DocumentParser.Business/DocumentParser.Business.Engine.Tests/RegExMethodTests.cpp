#include "DocumentParser/DocumentParser.Business/DocumentParser.Business.Engine.Tests/RegExMethodTests.h"

using DocumentParser::Business::Engine::Tests::RegExMethodTests;

void RegExMethodTests::SetUp()
{
	this->_searchMethod = std::shared_ptr<DPBE::RegExMethod>(new DPBE::RegExMethod());
...
void RegExMethodTests::TearDown()
{
}

DPI::Document* RegExMethodTests::GenerateDocument(std::string name, std::string content)
{
...
TEST_F(RegExMethodTests, InitializeSetsContent)
{
	std::string content = "test document. this document has minimal content. the reason is because i want to keep it on 1 line!";
	EXPECT_EQ(this->_searchMethod->GetContent(), content);
}

TEST_F(RegExMethodTests, ReturnsZeroIfEmptyFile)
{
...
TEST_F(RegExMethodTests, ReturnsZeroIfNotFound)
{
	std::string searchString = "abc";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 0);
}

TEST_F(RegExMethodTests, ReturnsOccurencesIfWordFound)
{
	std::string searchString = "test";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 1);
}

TEST_F(RegExMethodTests, ReturnsMultipleOccurencesIfWordFound)
{
	std::string searchString = "document";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 2);
}

TEST_F(RegExMethodTests, ReturnsOccurencesIfPhraseFound)
{
	std::string searchString = "ment. t";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 1);
}

TEST_F(RegExMethodTests, ReturnsMultipleOccurencesIfPhraseFound)
{
	std::string searchString = "ent. t";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 2);
}

TEST_F(RegExMethodTests, ReturnsOccurencesForNumbers)
{
	std::string searchString = "1";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 1);
}

TEST_F(RegExMethodTests, ReturnsOccurencesForSpecialCharacters)
{
	std::string searchString = "!";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 1);
}

std::string RegExMethodTests::GenerateRandomString() 
{
...
TEST_F(RegExMethodTests, DISABLED_PerformanceTest10Million)
{
	std::string content = 
...
TEST_F(RegExMethodTests, DISABLED_PerformanceTest2Million)
{
	std::string content = 
		"test document. this document has more content. the reason is because i want to test performance! "
		"To do so i am writing a longer document to parse through in hopes to gain a greater understanding "
...
TEST_F(RegExMethodTests, DISABLED_PerformanceTest10Thousand)
{
	std::string content = 
		"test document. this document has more content. the reason is because i want to test performance! "
		"To do so i am writing a longer document to parse through in hopes to gain a greater understanding "
