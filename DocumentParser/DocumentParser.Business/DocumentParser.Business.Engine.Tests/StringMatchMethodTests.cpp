#include "DocumentParser/DocumentParser.Business/DocumentParser.Business.Engine.Tests/StringMatchMethodTests.h"

using DocumentParser::Business::Engine::Tests::StringMatchMethodTests;

void StringMatchMethodTests::SetUp()
{
	this->_searchMethod = std::shared_ptr<DPBE::StringMatchMethod>(new DPBE::StringMatchMethod());
...
void StringMatchMethodTests::TearDown()
{
}

DPI::Document* StringMatchMethodTests::GenerateDocument(std::string name, std::string content)
{
...
TEST_F(StringMatchMethodTests, InitializeSetsContent)
{
	std::string content = "test document. this document has minimal content. the reason is because i want to keep it on 1 line!";
	EXPECT_EQ(this->_searchMethod->GetContent(), content);
}

TEST_F(StringMatchMethodTests, ReturnsZeroIfEmptyFile)
{
...
TEST_F(StringMatchMethodTests, ReturnsZeroIfNotFound)
{
	std::string searchString = "abc";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 0);
}

TEST_F(StringMatchMethodTests, ReturnsOccurencesIfWordFound)
{
	std::string searchString = "test";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 1);
}

TEST_F(StringMatchMethodTests, ReturnsMultipleOccurencesIfWordFound)
{
	std::string searchString = "document";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 2);
}

TEST_F(StringMatchMethodTests, ReturnsOccurencesIfPhraseFound)
{
	std::string searchString = "ment. t";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 1);
}

TEST_F(StringMatchMethodTests, ReturnsMultipleOccurencesIfPhraseFound)
{
	std::string searchString = "ent. t";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 2);
}

TEST_F(StringMatchMethodTests, ReturnsOccurencesForNumbers)
{
	std::string searchString = "1";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 1);
}

TEST_F(StringMatchMethodTests, ReturnsOccurencesForSpecialCharacters)
{
	std::string searchString = "!";
	EXPECT_EQ(this->_searchMethod->FindOccurences(searchString), 1);
}

std::string StringMatchMethodTests::GenerateRandomString() 
{
...
TEST_F(StringMatchMethodTests, PerformanceTest10Million)
{
	std::string content = 
...
TEST_F(StringMatchMethodTests, PerformanceTest2Million)
{
	std::string content = 
		"test document. this document has more content. the reason is because i want to test performance! "
		"To do so i am writing a longer document to parse through in hopes to gain a greater understanding "
...
TEST_F(StringMatchMethodTests, PerformanceTest10Thousand)
{
	std::string content = 
		"test document. this document has more content. the reason is because i want to test performance! "
		"To do so i am writing a longer document to parse through in hopes to gain a greater understanding "
